...
def load_data():
    ...
    sen_space=[]
    f = open('/home/jzhang/code/ChemTS/train_RNN/smiles_set.csv', 'rb')
    reader = csv.reader(f)
    for row in reader:
...
def save_model(model):
    ...
    with open("modelNH2under25.json", "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    model.save_weights("modelNH2under25.h5")
...
if __name__ == "__main__":
    ...
    model.fit(X,y_train_one_hot,nb_epoch=150, batch_size=512,validation_split=0.1)
    save_model(model)
